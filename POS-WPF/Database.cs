...
namespace POS_WPF
{
    public class Database
    {
        ...
        public List<Order> EndOfDay() //Gets a list of all orders from today
        {
            try
            {
                db.Open();
                ...
                command.CommandText = "SELECT `order_Num`, `order_TotalPrice`, `order_Info`, `employee_ID` FROM `order_history` WHERE date(`order_DateTime`)=CURRENT_DATE()";
                MySqlDataReader reader = command.ExecuteReader();
        ...
        public void PrintEndOfDay(Order totalOrders, int orderCount) //Creates the PDF for the end of Day Report
        {
            double height = 220;

            string text = "End Of Day Report\n" + DateTime.Now.ToString("g") + "\nNumber of Orders Today: " + orderCount + "\n\t";


            if (File.Exists("endOfDay.pdf"))
            {
                File.Delete("endOfDay.pdf");
            }

            Document document1 = new Document();
            document1.Info.Title = "End Of Day";
            ...
            row.Cells[2].AddParagraph("Amount Sold");

            foreach (OrderItem item in totalOrders.OrderItems)
            {
                row = table.AddRow();
                row.Cells[0].AddParagraph(item.ItemID.ToString());
                row.Cells[1].AddParagraph(item.Name);
            ...
            totalOrders.CalcTotalPrice();
            string totalRevenue = totalOrders.TotalPrice.ToString();
            if (totalRevenue.Split('.').Length < 2)
            {
                totalRevenue += ".00";
            }
            else
            {
                if (totalRevenue.Split('.')[1].Length < 2)
                {
                    totalRevenue += "0";
                }
            }

            Paragraph total = section1.AddParagraph("Total Revenue Today: £" + totalRevenue);
            ...
            renderer.PdfDocument.Save("endOfDay.pdf");

            Process.Start("endOfDay.pdf");
        }
        ...
        public bool ChangeStock(ref Item theItem, int amountToAdd) //Changes the amount in stock for an item
        {
            try
            {
                db.Open();
        ...
        public void PrintReceipt(Order theOrder, int orderNum, User currUser)  //Creates a PDF receipt for this order
        {
            double height = 240;
            ...
            Process.Start("LastOrderReceipt.pdf");

        }          

        public Order FindOrder(int OrderNum) //Searches the database for the order by orderNumber
        {
            try
            {
                db.Open();
        ...
        public List<Order> FindOrders(OrderIdentifiers searchFor, string searchTerm) //Searches the database for a list of orders by a searchterm
        {
            try
            {
                db.Open();
        ...
        public void GetItemInfo(ref Item theItem)
        {
            try
            {
                db.Open();
                ...
                command.CommandText = "SELECT `item_Name`,`item_Stock`,`item_Price` FROM `stock` WHERE `item_ID`='" + theItem.ID + "'";
                MySqlDataReader reader = command.ExecuteReader();

                if (reader.Read())
                {
                    theItem.Name = reader.GetString(0);
                    theItem.Stock = reader.GetInt32(1);
                    theItem.Price = reader.GetDouble(2);
                    theItem.Price = Math.Round(theItem.Price * 100) / 100;
                }
